"""BatchNorm Freezing experiments: performance, condition number,
weight updates, and latent norms.

Runs the same experiments as exper/exp1-4 but with the BF model.
"""
import os
...
def create_bf_model(key, depth, width=WIDTH, act_fn="tanh"):
    return FCResNetBN(
...
def run_exp1_performance():
    save_dir = ensure_dir(os.path.join(BF_RESULTS_DIR, "exp1"))
    print("\n" + "="*60)
    print("BF Experiment 1: Performance")
    print("="*60)

    for act_fn in ACT_FNS:
        depth_results = {}
        for depth in DEPTHS:
            print(f"\n--- BF ResNet | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_bf_model(key, depth=depth, act_fn=act_fn)
...
def run_exp2_condition():
    COND_WIDTH = 16
    save_dir = ensure_dir(os.path.join(BF_RESULTS_DIR, "exp2"))
    print("\n" + "="*60)
    print("BF Experiment 2: Condition Numbers")
    print("="*60)
    ...
    for act_fn in ACT_FNS:
        results = {}
        config_label = "BF ResNet"
        cond_nums = []
        for depth in DEPTHS:
            print(f"  {act_fn} | BF | depth={depth} ... ", end="", flush=True)
            key = jr.PRNGKey(SEED)
            model = create_bf_model(key, depth=depth, width=COND_WIDTH,
                                    act_fn=act_fn)
            try:
                cond, _ = compute_condition_number_bf(model, x_sample, y_sample)
        ...
        plot_condition_numbers(
            results, DEPTHS,
            title=f"Condition Number — {act_fn} (BF)",
    ...
    print(f"\nBF Exp 2 done. Results in {save_dir}")
...
def run_exp3_weight_updates():
    save_dir = ensure_dir(os.path.join(BF_RESULTS_DIR, "exp3"))
    print("\n" + "="*60)
    print("BF Experiment 3: Weight Updates")
    print("="*60)
    ...
    for act_fn in ACT_FNS:
        for depth in DEPTHS:
            print(f"\n--- BF Weight Updates | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_bf_model(key, depth=depth, act_fn=act_fn)
            ...
            plot_weight_updates(
                ...
                title=f"BF Weight Updates — {act_fn}, depth={depth}",
    ...
    plot_weight_updates_summary(
        summary, DEPTHS,
        title="BF Mean Weight Update Norm vs Depth",
        save_path=os.path.join(save_dir, "weight_updates_summary.png")
    )
    print(f"\nBF Exp 3 done. Results in {save_dir}")
...
def run_exp4_latent_norms():
    save_dir = ensure_dir(os.path.join(BF_RESULTS_DIR, "exp4"))
    print("\n" + "="*60)
    print("BF Experiment 4: Latent Norms")
    print("="*60)

    for act_fn in ACT_FNS:
        init_norms_by_depth = {}
        for depth in DEPTHS:
            print(f"\n--- BF Latent Norms | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_bf_model(key, depth=depth, act_fn=act_fn)
            ...
            plot_latent_norms_vs_layer(
                ...
                title=f"BF Activity Norms vs Layer — {act_fn}, depth={depth}",
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"BF Init Norms During Training — {act_fn}, depth={depth}",
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"BF Post-Inference Norms — {act_fn}, depth={depth}",
        ...
        plot_latent_norms_across_depths(
            init_norms_by_depth,
            title=f"BF Init Norms Across Depths — {act_fn}",
            save_path=os.path.join(save_dir, f"norms_across_depths_{act_fn}.png")
        )
    print(f"\nBF Exp 4 done. Results in {save_dir}")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("All BF experiments complete!")
    print(f"Results in {BF_RESULTS_DIR}")
    print("="*60)
