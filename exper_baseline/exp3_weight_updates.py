"""Experiment 3 (Baseline): Weight update magnitude during training.
...
def main():
    ...
    for act_fn in ACT_FNS:
        for depth in DEPTHS:
            ...
            plot_weight_updates(
                ...
                title=f"Weight Update Norms — {act_fn}, depth={depth} (MLP, no skip)",
    ...
    plot_weight_updates_summary(
        summary, DEPTHS,
        title="Mean Weight Update Norm vs Depth (MLP, no skip)",
        save_path=os.path.join(save_dir, "weight_updates_summary.png")
    )

    print(f"\nExperiment 3 (baseline) complete. Results saved to {save_dir}")
