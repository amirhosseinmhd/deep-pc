...
This is the OLD architecture where the skip connection is INSIDE the layer
call. jpc sees each layer as one function, so prediction errors are exactly
0 after ffwd init. An optional activity_noise parameter injects noise into
the activities after init to test whether that bootstraps learning.
"""
...
class DyTV2Variant:
    ...
    Supports optional activity noise injection to test whether small
    perturbations (simulating float32 rounding residuals) bootstrap learning.
    """
    ...
    def init_activities(self, model, x_batch):
        # Manual forward pass (old behavior — skip is inside each layer)
        activities = []
        h = x_batch
        ...
        # Optional noise injection into hidden activities
        if self.activity_noise > 0.0:
            for i in range(1, len(activities) - 1):
                self._noise_key, subkey = jr.split(self._noise_key)
                noise = self.activity_noise * jr.normal(
                    subkey, activities[i].shape
                )
                activities[i] = activities[i] + noise
