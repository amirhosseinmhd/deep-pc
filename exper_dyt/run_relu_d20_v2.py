"""Re-run relu depth=20 with DyT — all metrics."""
import os
...
from common_dyt import (
    ...
    DYT_RESULTS_DIR, _selected_layer_indices,
)
...
PARAM_LR = 5e-03
ACTIVITY_LR = 1e-03
save_dir = ensure_dir(os.path.join(DYT_RESULTS_DIR, "exp4_v2_d40"))
act_fn, depth = "relu", 20

# ================================================================
# 1. Performance + Weight Updates + Activity Norms (single run)
# ================================================================
print(f"\n=== DyT | {act_fn} | depth={depth} ===")
set_seed(SEED)
key = jr.PRNGKey(SEED)
model = FCResNetDyT(
    key=key, in_dim=INPUT_DIM, width=WIDTH, depth=depth,
    out_dim=OUTPUT_DIM, act_fn=act_fn, init_alpha=0.5
)
...
plot_latent_norms_vs_layer(
    ...
    title=f"DyT Norms vs Layer — {act_fn}, depth={depth}",
...
plot_latent_norms_vs_training(
    ...
    title=f"DyT Init Norms — {act_fn}, depth={depth}",
...
key = jr.PRNGKey(SEED)
cond_model = FCResNetDyT(
    key=key, in_dim=INPUT_DIM, width=COND_WIDTH, depth=depth,
    out_dim=OUTPUT_DIM, act_fn=act_fn, init_alpha=0.5
)
try:
    cond, _ = compute_condition_number_dyt(cond_model, x_sample, y_sample)
    print(f"  κ(H_z) = {cond:.2e}")
    results = {"DyT ResNet": [cond]}
    plot_condition_bar(
        results, [depth],
