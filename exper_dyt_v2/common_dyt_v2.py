"""Dynamic Tanh v2 (DyT-v2) for Predictive Coding Networks.

Same as DyT but with different DyT placement in hidden layers:
  - DyT  (original): x -> act_fn -> DyT(linear(x) + skip)
  - DyT-v2 (this):   x -> act_fn -> DyT(linear(x)) + skip

DyT wraps only the linear output; the skip connection is added AFTER
the dynamic tanh.  Everything else (DyT layer, output layer, training
loop) is identical to the original DyT experiment.
"""
import os
...
DYT_V2_RESULTS_DIR = os.path.join(os.path.dirname(__file__), "results")


# ============================================================================
# DyT Layer (identical to original DyT)
# ============================================================================
...
# ============================================================================
# Layer modules — v2 placement: DyT wraps linear only, skip added after
# ============================================================================
...
class HiddenLayerDyT(eqx.Module):
    ...
    DyT squashes the linear output BEFORE the skip connection is added.
    """
    ...
    def __call__(self, x):
        h = self.act_fn(x)
        h = self.dyt(self.linear(h)) + x   # DyT on linear only, then skip
        return h


class OutputLayer(eqx.Module):
    """Output layer: act_fn -> DyT -> Linear (no skip, same as original)."""
    act_fn: Callable = eqx.static_field()
    dyt: DyTLayer
    linear: nn.Linear
...
class FCResNetDyT_v2(eqx.Module):
    """FC-ResNet with DyT wrapping linear only (skip added after DyT)."""
...
# ============================================================================
def init_activities_dyt_v2(model, x_batch):
...
# Evaluation
# ============================================================================
def evaluate_dyt_v2(model, test_loader):
...
# ============================================================================
# DyT-v2 Training loop
# ============================================================================
def train_dyt_v2_and_record(
    seed, model, depth,
    ...
    """Train a DyT-v2 PCN and record metrics."""
    set_seed(seed)
    ...
    for iter_num in range(n_train_iters):
        ...
        activities = init_activities_dyt_v2(model, img_batch)
        ...
        if ((iter_num + 1) % test_every) == 0:
            avg_acc = evaluate_dyt_v2(model, test_loader)
...
# Condition number
# ============================================================================
def compute_condition_number_dyt_v2(model, x, y):
    """Compute kappa(H_z) for a DyT-v2 model at init."""
    activities = init_activities_dyt_v2(model, x)
