"""DyT v2 (Dynamic Tanh) experiments: performance, condition number,
weight updates, and latent norms.

Same experiments as DyT but with DyT wrapping linear only (skip added after DyT).
"""
import os
...
from common_dyt_v2 import (
    FCResNetDyT_v2, train_dyt_v2_and_record, compute_condition_number_dyt_v2,
    INPUT_DIM, OUTPUT_DIM, WIDTH, SEED,
    ACTIVITY_LR, PARAM_LR, BATCH_SIZE, N_TRAIN_ITERS, TEST_EVERY,
    DEPTHS, ACT_FNS, DYT_V2_RESULTS_DIR,
...
def create_dyt_v2_model(key, depth, width=WIDTH, act_fn="tanh", init_alpha=0.5):
...
def run_exp1_performance():
    save_dir = ensure_dir(os.path.join(DYT_V2_RESULTS_DIR, "exp1"))
    print("\n" + "="*60)
    print("DyT-v2 Experiment 1: Performance")
    print("="*60)

    for act_fn in ACT_FNS:
        depth_results = {}
        for depth in DEPTHS:
            print(f"\n--- DyT-v2 ResNet | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_dyt_v2_model(key, depth=depth, act_fn=act_fn)

            res = train_dyt_v2_and_record(
        ...
        plot_performance_curves(
            depth_results,
            title=f"DyT-v2 ResNet Performance — {act_fn}",
            save_path=os.path.join(save_dir, f"performance_{act_fn}.png")
        )
        plot_train_loss(
            depth_results,
            title=f"DyT-v2 ResNet Train Loss — {act_fn}",
            save_path=os.path.join(save_dir, f"train_loss_{act_fn}.png")
        )
    print(f"\nDyT-v2 Exp 1 done. Results in {save_dir}")
...
def run_exp2_condition():
    COND_WIDTH = 16
    save_dir = ensure_dir(os.path.join(DYT_V2_RESULTS_DIR, "exp2"))
    print("\n" + "="*60)
    print("DyT-v2 Experiment 2: Condition Numbers")
    print("="*60)
    ...
    for act_fn in ACT_FNS:
        results = {}
        config_label = "DyT-v2 ResNet"
        cond_nums = []
        for depth in DEPTHS:
            print(f"  {act_fn} | DyT-v2 | depth={depth} ... ", end="", flush=True)
            key = jr.PRNGKey(SEED)
            model = create_dyt_v2_model(key, depth=depth, width=COND_WIDTH,
                                        act_fn=act_fn)
            try:
                cond, _ = compute_condition_number_dyt_v2(model, x_sample, y_sample)
        ...
        plot_condition_numbers(
            results, DEPTHS,
            title=f"Condition Number — {act_fn} (DyT-v2)",
    ...
    print(f"\nDyT-v2 Exp 2 done. Results in {save_dir}")
...
def run_exp3_weight_updates():
    save_dir = ensure_dir(os.path.join(DYT_V2_RESULTS_DIR, "exp3"))
    print("\n" + "="*60)
    print("DyT-v2 Experiment 3: Weight Updates")
    print("="*60)
    ...
    for act_fn in ACT_FNS:
        for depth in DEPTHS:
            print(f"\n--- DyT-v2 Weight Updates | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_dyt_v2_model(key, depth=depth, act_fn=act_fn)
            ...
            plot_weight_updates(
                ...
                title=f"DyT-v2 Weight Updates — {act_fn}, depth={depth}",
    ...
    plot_weight_updates_summary(
        summary, DEPTHS,
        title="DyT-v2 Mean Weight Update Norm vs Depth",
        save_path=os.path.join(save_dir, "weight_updates_summary.png")
    )
    print(f"\nDyT-v2 Exp 3 done. Results in {save_dir}")
...
def run_exp4_latent_norms():
    save_dir = ensure_dir(os.path.join(DYT_V2_RESULTS_DIR, "exp4"))
    print("\n" + "="*60)
    print("DyT-v2 Experiment 4: Latent Norms")
    print("="*60)

    for act_fn in ACT_FNS:
        init_norms_by_depth = {}
        for depth in DEPTHS:
            print(f"\n--- DyT-v2 Latent Norms | {act_fn} | depth={depth} ---")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
            model = create_dyt_v2_model(key, depth=depth, act_fn=act_fn)
            ...
            plot_latent_norms_vs_layer(
                ...
                title=f"DyT-v2 Activity Norms vs Layer — {act_fn}, depth={depth}",
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"DyT-v2 Init Norms During Training — {act_fn}, depth={depth}",
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"DyT-v2 Post-Inference Norms — {act_fn}, depth={depth}",
        ...
        plot_latent_norms_across_depths(
            init_norms_by_depth,
            title=f"DyT-v2 Init Norms Across Depths — {act_fn}",
            save_path=os.path.join(save_dir, f"norms_across_depths_{act_fn}.png")
        )
    print(f"\nDyT-v2 Exp 4 done. Results in {save_dir}")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("All DyT-v2 experiments complete!")
    print(f"Results in {DYT_V2_RESULTS_DIR}")
    print("="*60)
