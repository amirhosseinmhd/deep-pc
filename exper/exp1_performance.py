"""Experiment 1: SP ResNet performance across depths and activations.

Trains standard-parameterisation (SP) ResNets on MNIST for ~2 epochs
and records train loss + test accuracy for depths {10, 20, 40} x {tanh, relu}.
"""
import os
...
def main():
    ...
    for act_fn in ACT_FNS:
        depth_results = {}
        for depth in DEPTHS:
            print(f"\n=== SP ResNet | act_fn={act_fn} | depth={depth} ===")
            set_seed(SEED)
            key = jr.PRNGKey(SEED)
        ...
        plot_performance_curves(
            depth_results,
            title=f"SP ResNet Performance — {act_fn}",
            save_path=os.path.join(save_dir, f"performance_{act_fn}.png")
        )
        plot_train_loss(
            depth_results,
            title=f"SP ResNet Train Loss — {act_fn}",
            save_path=os.path.join(save_dir, f"train_loss_{act_fn}.png")
        )

    print(f"\nExperiment 1 complete. Results saved to {save_dir}")
