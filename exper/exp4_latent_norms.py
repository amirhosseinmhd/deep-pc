"""Experiment 4: Norm of latent activities.
...
def main():
    ...
    for act_fn in ACT_FNS:
        ...
        for depth in DEPTHS:
            ...
            # activity_norms_init: (n_iters, n_activities)
            # activity_norms_post: (n_iters, n_activities)
            ...
            # --- Plot 1: norms vs layer at first iteration (init) ---
            first_init = norms_init[0]  # shape: (n_activities,)
            init_norms_by_depth[depth] = first_init

            plot_latent_norms_vs_layer(
                {"init (pre-inference)": first_init,
                 "post-inference": norms_post[0]},
                title=f"Activity Norms vs Layer — {act_fn}, depth={depth}",
                save_path=os.path.join(
                    save_dir, f"norms_vs_layer_{act_fn}_d{depth}.png"
                )
            )

            # --- Plot 2: norms vs training iteration for selected layers ---
            n_activities = norms_init.shape[1]
            layer_idxs = _selected_layer_indices(depth)
            # Map to activity indices (activities have depth-1 entries)
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"Init Activity Norms During Training — {act_fn}, depth={depth}",
                save_path=os.path.join(
            ...
            plot_latent_norms_vs_training(
                ...
                title=f"Post-Inference Activity Norms — {act_fn}, depth={depth}",
                save_path=os.path.join(
        ...
        # --- Plot 3: init norms across depths ---
        plot_latent_norms_across_depths(
            init_norms_by_depth,
            title=f"Init Activity Norms Across Depths — {act_fn} (SP ResNet)",
            save_path=os.path.join(
                save_dir, f"norms_across_depths_{act_fn}.png"
            )
        )

    print(f"\nExperiment 4 complete. Results saved to {save_dir}")
